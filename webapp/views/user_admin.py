...
from webapp.users_mgt import show_users, add_user
            ...
            dbc.Col([
                dt.DataTable(
                    ...
                    columns = [{'name' : 'ID', 'id' : 'id'},
                                ...
                                {'name' : 'Email', 'id' : 'email'},
                                {'name' : 'Admin', 'id' : 'admin'}],
                    data=[],  # Start with empty data, will be populated by callback
                ),
            ], md=12),
        ]),
    ], className='jumbotron')
...
################################################################################
# POPULATE USERS TABLE ON PAGE LOAD AND REFRESH PERIODICALLY
################################################################################
@app.callback(
    Output('users', 'data'),
...
def populate_users_table(n_intervals, create_success):
    """Populate the users table when the admin page is loaded or refreshed"""
    return show_users()
