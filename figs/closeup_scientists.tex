...
\begin{tikzpicture}[node distance=0.35cm, font=\sffamily,
    ...
    \node[expertcard, fill=cyan!8,
        ...
        {\textbf{Bioinformatics-}\\
         ...
         {\tiny scRNA-seq pipelines,}\\
         {\tiny WGCNA, SCENIC,}\\
         {\tiny multi-omics integ.,}\\
         {\tiny reproducibility}};
    ...
    \node[toolpanel, minimum width=19cm, text width=18cm,
        ...
        {\textbf{Shared Tool:}\;
         ...
         {\scriptsize Queries the specified expert's HyperRAG knowledge
          ...
          \textbf{LLM-only} expert reasons without private KB
          (methodology \& statistical critique).}\\[3pt]
...
In the example realisation shown here this compound agent runs fourth,
but the LLM planner may invoke it earlier, later, or re-invoke it
after a replan cycle---for instance, if literature search reveals
novel targets that warrant fresh expert synthesis.  It is the
heaviest reader of \texttt{AgentState}, consuming all prior omics,
KG, and literature output.  A PI-level router uses a structured
\texttt{ExpertRouting} Pydantic model to select 2--3 domain experts
and dispatch tailored sub-queries.  The system prompt distinguishes
\emph{RAG~KB loaded} experts (Genomics, Neuroscience,
Longevity/Biostatistics), which retrieve evidence from curated author
publications via HyperRAG, from the \emph{LLM-only}
BioinformaticsExpert, preferred for methodology and statistical
critique.  Routing rules mandate a bioinformatics perspective for
data-analysis tasks, a biostatistics perspective for statistical
claims, and all relevant experts for hypothesis generation (see
companion illustration for RAG initialisation and retrieval pipeline).
The PI synthesis node integrates expert perspectives into ranked
hypotheses with proposed validation experiments.}
